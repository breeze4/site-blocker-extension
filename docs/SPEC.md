...
- `timer-utils.js` — pure helpers (decrement, reset logic, URL/domain parsing, time formatting), unit-tested in isolation.
...
Options also allows changing the domain's time limit, recharge rate, and deleting the domain.
...
- A session ends — its duration added to `dailyTotals[today]` and `allTimeTotal` — when the user navigates away, switches tabs, closes the tab, the window loses focus, or the session goes idle.
...
- Global controls: the recharge-rate radios (30s/1m/2m/5m/10m/15m per hour, apply to all domains), and "Reset All Tracking".
...
- Shown only for a tracked domain with positive finite `timeLeft`. Hidden when the domain is untracked, or time has expired (the block/redirect logic handles that case).
...
Removed
- Global pause feature (`blockingPaused` / `pausePassword` / popup controls / Options panel) — removed because it was an all-or-nothing escape hatch that either went unused or defeated the whole extension. Replaced by earned per-domain reset tokens (see reset token spec).
- "Reset All Timers" button — removed alongside pause as an unconditional refill that required no effort. Replaced by the per-domain reset token as the single earned way to top up.
- Options link in the popup — was removed to harden the pause password friction. The pause feature is now gone, so the popup regains an Options link as a convenience.

Intentional frictions
- None at this time. The pause feature and its friction points have been removed; the reset token economy (specified separately) replaces them with earning-based gates.
