...
async function checkDomainExists(domain) {
  try {
    ...
    return Object.prototype.hasOwnProperty.call(domainTimers, domain);
  } catch (error) {
    return false;
  }
}
...
document.getElementById("urlInput").addEventListener("input", async (event) => {
  ...
  if (!input.trim()) {
    preview.innerHTML =
      '<span class="preview-placeholder">Enter a URL or domain to see what will be tracked</span>';
    return;
  }
...
async function renderDomainList() {
  ...
  try {
    ...
    existingRows.forEach((row) => {
      ...
      if (domain) {
        const timeRadio = row.querySelector(
